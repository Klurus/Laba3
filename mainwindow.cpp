...
void MainWindow::on_bookdetails_clicked()
{
    ...
    data[4] = model.data(model.index(selectedRows.at(0).row(), 5)).toString();
    emit sendData(data);
    changeBook.show();
    changeBook.exec();
...
void MainWindow::on_removebook_clicked()
{
    ...
    if (!selectedRows.isEmpty()) {
        model.removeRow(selectedRows.at(0).row());
        model.submit();
        selfModel.setSourceModel(&model);
        ui->booktable->setModel(&selfModel);
    }
...
void MainWindow::reciveSignaltoAddBook(QString *data)
{
    ...
    if(!strList.contains(data[0]))
    {
        ...
        model.setData(model.index(model.rowCount()-1, 5), data[4]);
        model.submit();
    }
    else{
        QMessageBox msgBox;
...
void MainWindow::importBooks(){
    ...
    for (const QJsonValue &value : dataArray)
    {
        ...
        model.setData(model.index(model.rowCount()-1, 5), item["Статус"].toString());
...
void MainWindow::exportBooks(){
    ...
    for (int row = 0; row < ui->booktable->model()->rowCount(); ++row)
    {
        ...
        index = ui->booktable->model()->index(row, 5);
        data = ui->booktable->model()->data(index, Qt::DisplayRole);
        jsonObj["Статус"] = data.toString();
